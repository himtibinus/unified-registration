...
@section('content')
    <form action="/events/{{ $event->id }}" method="POST" class="row flex-row-reverse justify-content-center mx-0"
        ...
        <div class="col-12 col-xl-4 p-4 pr-sm-0">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body text-dark">
                    <div class="row">
                        ...
                        <div class="col-12 col-sm-4 col-xl-12">
                            <p class="h4 font-700">Offline Seats</p>
                            <p class="display-4">{{ $event->offline_attandance }}/{{ $event->offline_seats }}</p>
                        </div>
                        <div class="col-12 col-sm-4 col-xl-12">
                            <p class="h4 font-700">Online Seats</p>
                            <p class="display-4">
                                {{ $event->online_attandance }}/{{ $event->seats - $event->offline_seats }}</p>
                        </div>
                        <div class="col-12 col-sm-4 col-xl-12">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body text-dark">
                    <div class="form-group mb-4">
                        ...
                        <input name="action-update-kicker" id="action-update-kicker" type="text"
                            value="{{ $event->kicker }}" class="form-control"
                            @if (!$role->admin) disabled @endif>
                    </div>
                    <div class="form-group mb-4">
                        <label for="action-update-name">{{ __('Event Name') }}<b class="text-danger">*</b></label>
                        <input name="action-update-name" id="action-update-name" type="text" value="{{ $event->name }}"
                            class="form-control" @if (!$role->admin) disabled @else required @endif>
                    </div>
                    <div class="form-group mb-4">
                        <label for="action-update-date">{{ __('Event Date') }}<b class="text-danger">*</b></label>
                        <input name="action-update-date" id="action-update-location" type="datetime-date"
                            pattern="[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\+[0-9]{2}:[0-9]{2}"
                            value="{{ date('c', strtotime($event->date)) }}" class="form-control"
                            @if (!$role->admin) disabled @else required @endif>
                    </div>
                    <div class="form-group mb-4">
                        <label for="action-update-location">{{ __('Event Location') }}</label>
                        <input name="action-update-location" id="action-update-location" type="text"
                            value="{{ $event->location }}" class="form-control"
                            @if (!$role->admin) disabled @endif>
                    </div>
                    <div class="form-group mb-4">
                        <label for="action-update-price">{{ __('Event Price (Rp)') }}<b
                                class="text-danger">*</b></label>
                        <input name="action-update-price" id="action-update-price" type="number" min="0"
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input name="action-update-theme_color_foreground" id="action-update-theme_color_foreground"
                            type="text" value="{{ $event->theme_color_foreground }}" class="form-control"
                            @if (!$role->admin) disabled @endif>
                    </div>
                    <div class="form-group mb-4">
                        <label for="action-update-theme_color_background">{{ __('Background Color') }}</label>
                        <input name="action-update-theme_color_background" id="action-update-theme_color_background"
                            type="text" value="{{ $event->theme_color_background }}" class="form-control"
                            @if (!$role->admin) disabled @endif>
                    </div>
                </div>
            </div>
            <div class="card mb-4">
                ...
                <div class="card-body text-dark">
                    <p>This description will be publicly visible towards all users, registered or not. <b>Markdown is
                            supported and enabled by default.</b></p>
                    <div class="form-group mb-4">
                        <textarea name="action-update-description_public" id="action-update-description_public" type="text" rows="4"
                            class="form-control"
                            @if (!$role->admin) disabled @endif>{{ $event->description_public }}</textarea>
                    </div>
                </div>
            </div>
            <div class="card mb-4">
                ...
                <div class="card-body text-dark">
                    <p>This description will be visible to users who has been recently registered, such as placing payment
                        information. <b>Markdown is supported and enabled by default.</b></p>
                    <div class="form-group mb-4">
                        <textarea name="action-update-description_pending" id="action-update-description_pending" type="text" rows="4"
                            class="form-control"
                            @if (!$role->admin) disabled @endif>{{ $event->description_pending }}</textarea>
                    </div>
                </div>
            </div>
            <div class="card mb-4">
                ...
                <div class="card-body text-dark">
                    <p>This description will be visible to users who has been accepted to the event. <b>Markdown is
                            supported and enabled by default.</b></p>
                    <div class="form-group mb-4">
                        <textarea name="action-update-description_private" id="action-update-description_private" type="text" rows="4"
                            class="form-control"
                            @if (!$role->admin) disabled @endif>{{ $event->description_private }}</textarea>
                    </div>
                </div>
            </div>
            <div class="card mb-4">
                ...
                <div class="card-body text-dark">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input name="action-update-seats" id="action-update-seats" type="number"
                            value="{{ $event->seats }}" min="1" class="form-control"
                            @if (!$role->admin) disabled @else required @endif>
                    </div>
                    <div class="form-group mb-4">
                        <label for="action-update-offline_seats">{{ __('Maximum Offline Available Seats') }}</label>
                        <input name="action-update-offline_seats" id="action-update-offline_seats" type="number"
                            value="{{ $event->offline_seats }}" min="0" class="form-control"
                            @if (!$role->admin) disabled @else required @endif>
                    </div>
                    <div class="form-group mb-4">
                        <label for="action-update-slots">{{ __('Maximum Tickets per User') }}</label>
                        <input name="action-update-slots" id="action-update-slots" type="number"
                            value="{{ $event->slots }}" min="1" class="form-control"
                            @if (!$role->admin) disabled @else required @endif>
                    </div>
                    <div class="form-group mb-4">
                        <label for="action-update-team_members">{{ __('Required Team Members') }}</label>
                        <input name="action-update-team_members" id="action-update-team_members" type="number"
                            value="{{ $event->team_members }}" min="0" class="form-control"
                            @if (!$role->admin) disabled @else required @endif>
                    </div>
                    <div class="form-group mb-4">
                        <label for="action-update-team_members_reserve">{{ __('Reserve Team Members') }}</label>
                        <input name="action-update-team_members_reserve" id="action-update-team_members_reserve"
                            type="number" value="{{ $event->team_members_reserve }}" min="0" class="form-control"
                            @if (!$role->admin) disabled @else required @endif>
                    </div>
                    <div class="form-group mb-4">
                        <label for="action-update-payment_link">{{ __('Payment URL') }}</label>
                        <input name="action-update-payment_link" id="action-update-payment_link" type="url"
                            value="{{ $event->payment_link }}" min="0" class="form-control"
                            @if (!$role->admin) disabled @endif>
                    </div>
                </div>
            </div>
            <div class="card mb-4">
                ...
                <div class="card-body text-dark">
                    ...
                    <div class="form-group mb-4">
                        <label
                            for="action-update-url_link"><b>{{ __('URL for Online Participants') }}</b><br>Participants
                            will be redirected to this link after check in.</label>
                        <input name="action-update-url_link" id="action-update-url_link" type="text"
                            value="{{ $event->url_link }}" class="form-control"
                            @if (!$role->admin) disabled @endif>
                    </div>
                    <div class="form-group mb-4">
                        <label for="action-update-totp_key"><b>{{ __('Event Check Out Token') }}</b><br>Online
                            participants will be propted to enter the following token to check out.</label>
                        <input name="action-update-totp_key" id="action-update-totp_key" type="text"
        ...
        <div class="col-12 col-xl-8 p-4 pb-sm-0">
            ...
            @foreach ($registrations as $registration)
                <div class="card mb-4">
                    <div
                        class="card-header h4 @if ($registration->status > 4) bg-primary @elseif($registration->status > 3) bg-info @elseif($registration->status > 1) bg-success @elseif($registration->status == 1) bg-danger @else bg-secondary @endif text-white">
                        #{{ $registration->id }}: {{ $registration->name }} @if ($registration->remarks == 'Late')
                            <span class="badge rounded-pill bg-light text-dark">Late</span>
                        @endif
                    </div>
                    <div class="card-body text-dark">
                        <p class="card-title">
                            ...
                            <br>
                            <b>Offline Status:</b>
                            @switch($registration->offline_status)
                                @case(0)
                                    Online
                                @break
                        ...
                        <div class="btn-toolbar" role="toolbar">
                            <div class="btn-group mr-2" role="group">
                                <button type="button" class="btn btn-primary"
                                    onClick="requestParticipantDetails('{{ $registration->email }}')">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="participantDetailsModalLabel"><i class="bi bi-person-circle"></i>
